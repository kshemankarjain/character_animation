...
UNITY_INSTANCING_BUFFER_END(Terrain)
      ...
      MicroSplatLayer SurfImpl(Input i, float3 worldNormalVertex)
      {
         ...
         #if _MSRENDERLOOP_SURFACESHADER && !_DEBUG_USE_TOPOLOGY && !_TERRAINBLENDABLESHADER && (defined(UNITY_INSTANCING_ENABLED) || _PERPIXNORMAL)
            #if _MICROTERRAIN
            {
               // when using per pixel normal via instancing or the feature, the vertex shader sets the normal
               // to 0,1,0. So when the vertex shader goes to compute view dir, it gets the wrong TBN matrix.
               // So we have to construct the right one here and convert to tsviewdir. 
...
ENDCG
   ...
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_Base-815888286"
