...
namespace JBooth.MicroSplat
{
   ...
   public class MicroSplatBaseFeatures : FeatureDescriptor
   {
      ...
      GUIContent CNormalMode = new GUIContent("Normal Map Mode", "Allows you to set normals to be disabled, sampled and blended in a performant way, or using the high quality Surface Gradient framework");
      ...
      public override void Unpack(string[] keywords)
      {
         ...
         if (HasFeature(keywords, DefineFeature._NONOMALMAP))
         {
      ...
      public override void ComputeSampleCounts(string[] features, ref int arraySampleCount, ref int textureSampleCount, ref int maxSamples, ref int tessellationSamples, ref int depTexReadLevel)
      {
         ...
         {
            arraySampleCount += normalMode == NormalMode.Disabled ? 4 : 8;
            if (emissiveArray)
            {
               arraySampleCount += 4;
            }
         ...
         {
            arraySampleCount += normalMode == NormalMode.Disabled ? 3 : 6;
            if (emissiveArray)
            {
               arraySampleCount += 3;
            }
         ...
         {
            arraySampleCount += normalMode == NormalMode.Disabled ? 2 : 4;
            if (emissiveArray)
            {
               arraySampleCount += 2;
            }
